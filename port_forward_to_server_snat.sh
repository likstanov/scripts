...
# ==========================================
# DNAT + SNAT helper with de-dup & sysctl
# ==========================================

usage() {
  echo "Использование:"
  echo "  wget -qO- https://raw.githubusercontent.com/likstanov/scripts/refs/heads/main/port_forward_to_server_snat.sh | \\"
  echo "    bash -s -- --ip <IP> --remote-port <REMOTE_PORT> [--local-port <LOCAL_PORT>] [--protocol <tcp|udp|both>] [--snat-ip <SNAT_IP>] [--enable-ip-forward] [--static] [--delete]"
  echo
  ...
  echo "  --snat-ip:            Внешний IP для SNAT (по умолчанию автоопределение через ip route)"
...
detect_snat_ip() {
  ...
  if command -v ip >/dev/null 2>&1; then
    SNAT_IP=$(ip -4 route get 1.1.1.1 2>/dev/null | awk '/src/ {print $7; exit}')
  fi

  if [ -z "${SNAT_IP:-}" ]; then
    SNAT_IP=$(hostname -I 2>/dev/null | awk '{print $1}')
  fi

  if [ -z "${SNAT_IP:-}" ]; then
    echo "Не удалось автоматически определить внешний IP для SNAT. Укажи --snat-ip."
    exit 1
  fi

  FINAL_MESSAGE+="SNAT будет использовать внешний IP: $SNAT_IP\n"
}
...
rule_exists_exact() {
  ...
  if [ -n "$table_opt" ]; then
    iptables $table_opt -C "$@" 2>/dev/null
  else
...
dedup_rule() {
  ...
  if [ "$count" -gt 0 ]; then
    ...
    while [ $i -lt $to_remove ]; do
      if [ -n "$table_opt" ]; then
        iptables $table_opt -D "${args[@]}" || true
      else
        iptables -D "${args[@]}" || true
      fi
      i=$((i+1))
    done
...
snat_exists()   { rule_exists_exact "-t nat" POSTROUTING -p "$1" -d "$IP" --dport "$REMOTE_PORT" -j SNAT --to-source "$SNAT_IP"; }

dedup_dnat_set() {
  local proto=$1
  dedup_rule true "-t nat" PREROUTING  PREROUTING  -p "$proto" --dport "$LOCAL_PORT" -j DNAT --to-destination "$IP:$REMOTE_PORT"
  dedup_rule true ""        FORWARD    FORWARD    -p "$proto" -d "$IP" --dport "$REMOTE_PORT" -j ACCEPT
  dedup_rule true "-t nat" POSTROUTING POSTROUTING -p "$proto" -d "$IP" --dport "$REMOTE_PORT" -j SNAT --to-source "$SNAT_IP"
  # заодно подчистим старые MASQUERADE, если остались от предыдущей версии
  dedup_rule false "-t nat" POSTROUTING POSTROUTING -p "$proto" -d "$IP" --dport "$REMOTE_PORT" -j MASQUERADE
}
...
manage_rule() {
  ...
  if [ "$action" = "add" ]; then
    ...
    if ! snat_exists "$proto"; then
      iptables -t nat -A POSTROUTING -p "$proto" -d "$IP" --dport "$REMOTE_PORT" -j SNAT --to-source "$SNAT_IP"
      FINAL_MESSAGE+="POSTROUTING SNAT для $IP:$REMOTE_PORT ($proto) через $SNAT_IP добавлен.\n"
    else
      FINAL_MESSAGE+="POSTROUTING SNAT для $IP:$REMOTE_PORT ($proto) уже существует.\n"
    fi
    ...
    while snat_exists "$proto";    do iptables -t nat -D POSTROUTING -p "$proto" -d "$IP" --dport "$REMOTE_PORT" -j SNAT --to-source "$SNAT_IP" || break; done
    # подчистим старые MASQUERADE, если есть
    while rule_exists_exact "-t nat" POSTROUTING -p "$proto" -d "$IP" --dport "$REMOTE_PORT" -j MASQUERADE; do
      iptables -t nat -D POSTROUTING -p "$proto" -d "$IP" --dport "$REMOTE_PORT" -j MASQUERADE || break
    done
